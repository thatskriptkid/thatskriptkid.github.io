...
We found out that ```MortarGameLauncherActivity``` launches ```MortarGameActivity``` and closes. Open it and we are interested in what happens in the Oncreate method, since it starts the creation of activity. Immediately after it we will insert our code. It is important not to upset the line when pasting.
